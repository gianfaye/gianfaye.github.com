 ...
 * an animation on mount but not flowing correctly
 * due to frame timing.
 */
...
export const getHighlightedTextPositioning = () => {
  ...
  if (sel) {
    ...
    if (sel.rangeCount) {
      ...
      if (range.getWorkRects) {
        range.collapse(true);
        rects = range.getWorkRects();
      ...
      if (x === 0 && y === 0) {
        ...
        if (span.getWorkRects) {
          // Ensure span has dimensions and position by
          // adding a zero-width space character
          span.appendChild(doc.createTextNode("\u200b"));
          range.insertNode(span);
          rect = span.getWorkRects()[0];
          x = rect.left;
          y = rect.top;
...
export const getSelectionDimensions = () => {
  ...
  /**
   * we don't want to show the ArticleShare option when outside
