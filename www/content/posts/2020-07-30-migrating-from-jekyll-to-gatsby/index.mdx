---
title: "Migrating my website from Jekyll (Ruby) to Gatsby.js (React.js)"
slug: migrating-form-jekyll-to-gatsby
topic: Web Development
date: 2022-07-30
hero: ./images/migrating-jekyll-to-gatsby.jpg
excerpt: "How I migrated this website from Jekyll to GatsbyJS"
categories: ['Updates']
---

As a front-end developer, when I chanced upon the newly-coined yet-another-tech-jargon that is [JAMstack](https://jamstack.org/), I got intrigued. JAM stands for JavaScript, APIs, Markups. It is when you can build everything in JavaScript, server-side processes are abstracted into microservice APIs, and the Markup is pre-built using a static site generator. It's like being in control of everything for once. On a traditional web development project cycle, the frontend has always been stacked after UI and before backend development. But with the JAMstack, the need to manage and run web servers is entirely eliminated. Gone are the days that the frontend is mainly for the client-side and the backend for the server-side.

I started migrating my website on the last week of June this year. The previous version of my website is generated using [Jekyll](https://jekyllrb.com/) and deployed in Github Pages. It was first deployed in 2013 (7 years sure came by so fast). You can view it by visiting [jekyll.gianfaye.com](http://jekyll.gianfaye.com/).
...
It already has the basic foundation for publishing blogs with posts formatted in Markdown. You can either write posts on the theme repo itself or add them via [Contentful](https://www.contentful.com/) or [Netlify](https://www.netlify.com/). Since I like writing on my IDE, I am publishing via Github Pages with the option to migrate to Contentful when I reached the [Github repo size limit](https://docs.github.com/en/github/managing-large-files/what-is-my-disk-quota). Scalability, it matters.
...
* Custom taxonomy for blog posts: **category**
* Custom taxonomy for project: **project type**
...
Renaming is easy. But for the new post type **projects** where I needed to add a new taxonomy named **clients**, the following files should be updated as well:
...
And on the post itself, the field (topic or client) should have a value already listed on the `topics.yml` or `clients.yml`.
